...
- **Single Source of Truth (SSOT):** Critical global states (Settings, Admin Mode) must be managed centrally and synchronized across all components to prevent data divergence.
- **Wizard-First Form Policy:** Any form with more than 3 input fields MUST implement a step-by-step Wizard flow to ensure guided user experience.

## 2. Engineering Standards (Clean Code)

**Philosophy:** A sustainable, readable, and testable codebase.

- **Production-Ready Validation:** All technical edits must be validated via three mandatory pillars: `npm run type-check`, `npm run lint`, and `npm run build`.
