...
- **Wizard UX (Single-Mode Focus):** Break down complex interactions (creation, mass updates) into step-by-step Wizard flows. Each screen must have only ONE focus point to ensure clarity for all user age groups.
- **Async Feedback (Pulse/Spin):** Every asynchronous operation (verify, upload, save) MUST show a loading indicator (spinner or bar) within the active element to provide immediate visual confirmation of the process status.
