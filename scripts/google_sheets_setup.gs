...
function doPost(e) {
  try {
    ...
    if (action === 'GET_SETTINGS') {
      ...
      return ContentService.createTextOutput(
        JSON.stringify({ status: 'Success', data: settingsObj }),
      ).setMimeType(ContentService.MimeType.JSON);
    }
    ...
    if (action === 'DELETE_ALL') {
      ...
      return ContentService.createTextOutput('Success: Full Clear').setMimeType(
      ...
      for (var i = 1; i < rows.length; i++) {
        if (String(rows[i][0]) === String(id)) {
          if (changes.name !== undefined)
            sheet.getRange(i + 1, 2).setValue(changes.name);
          if (changes.category !== undefined)
            sheet.getRange(i + 1, 3).setValue(changes.category);
          if (changes.price !== undefined)
            sheet.getRange(i + 1, 4).setValue(changes.price);
          if (changes.image !== undefined)
            sheet.getRange(i + 1, 5).setValue(changes.image);
          if (changes.description !== undefined)
            sheet.getRange(i + 1, 6).setValue(changes.description);
          if (changes.inStock !== undefined)
            sheet.getRange(i + 1, 7).setValue(String(changes.inStock));
          if (changes.is_archived !== undefined)
            sheet.getRange(i + 1, 8).setValue(String(changes.is_archived));
          break;
        }
      }
      return ContentService.createTextOutput('Success: Updated').setMimeType(
        ContentService.MimeType.TEXT,
      );
    }
    ...
    if (action === 'DELETE') {
      ...
      return ContentService.createTextOutput('Success: Deleted').setMimeType(
        ContentService.MimeType.TEXT,
      );
    }
    ...
    if (action === 'ADD') {
      ...
      return ContentService.createTextOutput('Success: Added').setMimeType(
        ContentService.MimeType.TEXT,
      );
    }
    ...
    return ContentService.createTextOutput('Unknown Action').setMimeType(
      ContentService.MimeType.TEXT,
    );
  } catch (err) {
    return ContentService.createTextOutput(
