  ...
  return (
    <article className={`bg-white border ${product.inStock === false ? 'border-transparent bg-stone-50' : 'border-stone-200'} rounded-lg flex flex-col group hover:shadow-md transition-shadow duration-200 relative`}>
      ...
      <div className="px-2 py-2 flex flex-col gap-0.5 flex-grow">
        ...
        {/* Price & Actions (Delete + Stock Toggle) */}
        <div className="mt-auto flex items-center justify-between">
          ...
          {isAdmin && (
            <div className="flex gap-1.5 items-center">
              {/* Stock Toggle (Done) Button */}
              <button 
                onClick={(e) => { e.stopPropagation(); onUpdate(product.id, { inStock: product.inStock === false ? true : false }); }} 
                className={`w-6 h-6 flex items-center justify-center rounded-full transition-colors shadow ${product.inStock !== false ? 'bg-green-600 hover:bg-green-700 text-white' : 'bg-stone-300 hover:bg-stone-400 text-stone-600'}`} 
                aria-label="Stok Durumu"
                title={product.inStock !== false ? "Stokta (Kapatmak için tıkla)" : "Stok Tükendi (Açmak için tıkla)"}
              >
              ...
              {/* Delete Button */}
              <button onClick={handleDeleteClick} className="w-6 h-6 flex items-center justify-center rounded-full bg-red-600 text-white hover:bg-red-700 transition-colors shadow" aria-label="Ürünü sil" title="Ürünü Sil">
