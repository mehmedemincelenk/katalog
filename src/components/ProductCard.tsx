...
function MarqueeText({ text, textClass, isAdmin, editableProps = {} }) {
  ...
  useEffect(() => {
    ...
    if (!el) return;
    const check = () =>
      setOverflow(el.scrollWidth > el.clientWidth + CL.marqueeTolerance);
    check();
...
function DescriptionScroll({ lines, lineClass, maxHeightClass }) {
  ...
  useEffect(() => {
    ...
    if (!el) return;
    const check = () =>
      setOverflow(el.scrollHeight > el.clientHeight + CL.marqueeTolerance);
    check();
    const ro = new ResizeObserver(check);
    ro.observe(el);
    return () => ro.disconnect();
  ...
  const renderLines = (keyPrefix) =>
    lines.map((line, i) => (
      <div
        key={`${keyPrefix}-${i}`}
        className={`${lineClass} flex gap-1 items-baseline`}
      >
...
export default function ProductCard({
  ...
  return (
    ...
    >
      {/* 1. IMAGE SECTION */}
      <div
        className={`relative w-full bg-stone-100 aspect-square flex items-center justify-center rounded-t-lg ${isAdmin ? 'cursor-pointer' : ''}`}
            ...
            <div className="p-3 pt-1 bg-white">
              <div className="flex flex-wrap gap-1 mb-2 max-h-32 overflow-y-auto">
                {categories.map((cat) => (
                  <button
                    ...
                    className={`px-1.5 py-0.5 text-[9px] rounded-sm font-semibold border ${cat === product.category ? 'bg-kraft-100 text-kraft-800 border-kraft-300' : 'bg-stone-50 text-stone-600 border-stone-200'}`}
                  >
                    {cat}
                  </button>
                ))}
              ...
              <div className="flex gap-1">
                ...
                <button
                  ...
                  className="bg-stone-900 text-white px-2 py-0.5 rounded text-[10px] font-bold"
                >
                  +
                </button>
              </div>
          ...
          {showActions && (
            ...
            >
              <button
                ...
                className={`${CL.iconMedium} flex items-center justify-center rounded-full ${!product.inStock ? 'bg-stone-900 text-white' : 'text-stone-400 hover:bg-stone-100 hover:text-stone-700'}`}
                title="Stok Durumu"
              >
              ...
              <button
                ...
                className={`${CL.iconMedium} flex items-center justify-center rounded-full ${product.is_archived ? 'bg-stone-900 text-white' : 'text-stone-400 hover:bg-stone-100 hover:text-stone-700'}`}
                title="Arşivle"
              >
                <svg
                  xmlns="http://www.w3.org/2000/svg"
              ...
              <button
                onClick={() => (
                  setShowActions(false),
                  window.confirm('Silinsin mi?') && onDelete(product.id)
                )}
                className={`${CL.iconMedium} flex items-center justify-center rounded-full text-red-500 hover:bg-red-50`}
                title="Sil"
              >
                <svg
                  xmlns="http://www.w3.org/2000/svg"
