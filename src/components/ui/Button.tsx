...
export default function Button({
  ...
  return (
    ...
    >
      ...
      {showFingerprint && (
        <div
          className={`absolute opacity-10 pointer-events-none transition-transform duration-500 ${size === 'xs' || size === 'sm' ? '-right-2 -bottom-2' : '-right-6 -bottom-6'} ${selected ? 'opacity-20 scale-110' : 'group-hover:scale-110 group-hover:opacity-15'}`}
          style={{
            transform: `scaleX(-1) rotate(${fingerprintRotation ?? 10}deg)`,
          }}
        >
          <svg
            xmlns="http://www.w3.org/2000/svg"
            width={size === 'xs' || size === 'sm' ? '36' : '90'}
            height={size === 'xs' || size === 'sm' ? '36' : '90'}
            fill="currentColor"
            viewBox="0 0 16 16"
          >
