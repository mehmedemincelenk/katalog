...
const StatusOverlay = memo(
  ({ status, message, onClose, mode = 'fixed' }: StatusOverlayProps) => {
    ...
    return (
      <AnimatePresence>
        ...
        >
          ...
          >
            <div
              className={`w-24 h-24 rounded-full flex items-center justify-center shadow-2xl transition-colors duration-500 ${status === 'success' ? 'bg-emerald-500' : status === 'error' ? 'bg-red-500' : 'bg-stone-900'}`}
            >
              {status === 'success' && (
                <Lucide.Check
                  size={48}
