...
export const THEME = {
  ...
  button: {
    ...
    sizes: {
      circle: { xs: 'w-6 h-6', sm: 'w-9 h-9', md: 'w-11 h-11 sm:w-[66px] sm:h-[66px]', lg: 'w-14 h-14' },
      square: { xs: 'w-7 h-7 text-[10px]', sm: 'w-8 h-8 sm:w-[48px] sm:h-[48px] text-xs sm:text-base', md: 'w-10 h-10 text-sm', lg: 'w-12 h-12 text-base' },
      rectangle: { xs: 'px-2 py-1 text-[9px]', sm: 'px-4 py-2 text-[11px]', md: 'px-6 py-3 text-xs', lg: 'px-8 py-4 text-sm' }
    }
  },
  
  navbar: {
    layout: 'bg-white/80 backdrop-blur-xl border-b border-stone-100 sticky top-0 z-[100] h-14 sm:h-[86px]',
    container: 'w-full px-2 sm:px-[12px] lg:px-6 h-full flex items-center',
    innerWrapper: 'flex justify-between items-center w-full',
    brand: {
      wrapper: 'flex items-center gap-1.5 sm:gap-[10px] shrink-0',
      logoWrapper: 'flex items-center justify-center transition-all active:scale-90',
      logoImg: 'w-9 h-9 sm:w-[61px] sm:h-[61px] object-contain',
      logoText: FS.xl.replace('font-black', 'font-normal'),
      textWrapper: 'flex flex-col leading-tight sm:gap-1',
      name: `${FS.base} sm:text-[15px] tracking-tight`,
      tagline: `${FS.xs} sm:text-[10px] text-stone-400 font-semibold`,
  ...
  floatingAdminMenu: {
    wrapper: 'fixed bottom-2 right-2 sm:bottom-4 sm:right-4 z-[150] sm:scale-[2] sm:origin-bottom-right',
  ...
  heroCarousel: {
    ...
    navigation: {
      ...
      prevPos: 'left-4',
      nextPos: 'right-4',
      dotsWrapper: 'absolute bottom-4 left-1/2 -translate-x-1/2 flex gap-1.5 sm:gap-2.5 z-10',
  ...
  searchFilter: {
    layout: 'bg-white border-b border-stone-200 py-3 relative sticky top-[56px] sm:top-[86px] z-40',
    ...
    searchArea: {
      ...
      inputWrapper: `relative flex-1 sm:w-64 min-w-[140px] ${BR.low}`,
      iconSize: 'absolute left-3 sm:left-[18px] top-1/2 -translate-y-1/2 w-4 h-4 sm:w-6 sm:h-6 text-stone-400',
      input: `w-full pl-9 pr-8 sm:pl-[54px] sm:pr-12 py-2 sm:py-3 border border-stone-200 ${FS.sm} sm:text-[13px] text-stone-900 focus:ring-2 focus:ring-stone-900 outline-none transition-colors duration-200 bg-stone-50/50 ${BR.low} motion-fix`,
    ...
    categoryList: {
      ...
      chip: {
        ...
        adminSelectWrapper: 'relative w-8 sm:w-12 h-full bg-white border-r border-stone-200 flex items-center justify-center overflow-hidden',
        ...
        deleteIcon: 'ml-1 text-red-400 hover:text-red-600 font-black text-xs sm:text-lg transition-colors',
      },
  ...
  productGrid: {
    ...
    header: {
      wrapper: 'flex items-center gap-3 sm:gap-[13px] mb-6 sm:mb-6 px-1 sm:px-1.5',
  ...
  footer: {
    ...
    coupons: {
      wrapper: 'flex flex-col items-center gap-2 sm:gap-3 w-full max-w-[240px] sm:max-w-[360px]',
    ...
    bottomBar: {
      layout: 'w-full py-3 sm:py-[18px] bg-stone-900 flex items-center justify-center relative overflow-hidden group cursor-pointer',
  ...
  productCard: {
    ...
    typography: {
      name: `${FS.sm} sm:text-xl text-stone-900 leading-tight tracking-tight`,
      nameOutOfStock: 'opacity-40 text-stone-400',
      nameTransition: 'transition-all duration-300',
      description: `${FS.xs} sm:text-base text-stone-500 leading-snug font-medium normal-case`,
      descriptionClamp: 'line-clamp-2',
      descriptionFull: 'w-full',
      price: `${FS.base} sm:text-3xl font-black tracking-tight transition-all duration-500`,
  ...
  addProductModal: {
    ...
    container: `bg-white w-full max-w-sm sm:max-w-3xl ${BR.lg} shadow-2xl flex flex-col max-h-[85vh] animate-in zoom-in-95 duration-300`,
    header: 'flex items-center justify-between px-6 sm:px-10 py-5 sm:py-6 border-b border-stone-100',
    headerButton: `!w-11 !h-11 sm:!w-[70px] sm:!h-[70px] !bg-stone-100 !text-stone-500 hover:!bg-stone-200 hover:!text-stone-900 ${BR.low}`,
    ...
    footerCancel: 'flex-1 !border-2 !border-stone-100 !text-stone-400 !py-3.5 sm:!py-[22px] sm:text-[18px]',
    footerSubmit: 'flex-[1.5] !py-3.5 sm:!py-[22px] sm:text-[18px]',
    inputField: `w-full border border-stone-200 ${BR.low} px-3 py-2.5 sm:px-5 sm:py-4 ${FS.sm} sm:text-[16px] focus:outline-none focus:ring-2 focus:ring-stone-900 transition outline-none bg-stone-50/50`,
    stockToggle: `flex items-center justify-between bg-stone-50 border border-stone-200 ${BR.low} px-4 py-3.5 sm:px-6 sm:py-5 shadow-sm`,
    checkbox: `w-5 h-5 sm:w-8 sm:h-8 text-stone-900 border-stone-300 rounded-[0.2rem] focus:ring-0 cursor-pointer shadow-sm`,
    categoryChipActive: `bg-stone-900 text-white border-stone-900 px-3 py-1.5 sm:px-5 sm:py-2.5 ${FS.xs} sm:text-[13px] font-bold ${BR.full} transition-all shadow-sm`,
    categoryChipInactive: `bg-white text-stone-600 border-stone-200 hover:border-stone-400 active:scale-95 px-3 py-1.5 sm:px-5 sm:py-2.5 ${FS.xs} sm:text-[13px] font-bold ${BR.full} transition-all shadow-sm`,
    categoryShowMore: `${BR.full} px-3 py-1.5 sm:px-6 sm:py-3 ${FS.xs} sm:text-base font-bold border border-dashed border-stone-300 text-stone-500 hover:border-stone-500 hover:text-stone-700 transition-all bg-stone-50 active:scale-95`,
    wizard: {
      progressWrapper: 'flex gap-1 sm:gap-2 mt-1 sm:mt-2',
      progressBase: 'h-1 sm:h-2 w-6 sm:w-12 rounded-full transition-all duration-500',
      stepActive: 'bg-kraft-600',
      stepInactive: 'bg-stone-200',
      stepContent: 'space-y-6 sm:space-y-12 animate-in fade-in slide-in-from-bottom-2 duration-500',
    },
    typography: {
      ...
      errorBadge: `bg-red-50 text-red-600 ${FS.xs} sm:text-lg font-bold uppercase p-3 sm:p-6 ${BR.low} text-center border border-red-100 shadow-sm`,
    },
    imagePicker: {
  ...
  modal: {
    ...
    closeButtonWrapper: 'absolute top-6 sm:top-10 right-6 sm:right-10 z-[210] sm:scale-[1.6]',
    closeButtonCustom: '!bg-white/80',
    imageWrapper: 'w-full flex items-center justify-center max-h-[50vh] md:max-h-[65vh]',
    image: `max-w-full max-h-[50vh] md:max-h-[65vh] object-contain shadow-2xl ${BR.lg}`,
    contentWrapper: 'text-center space-y-4 sm:space-y-6 w-full max-w-2xl sm:max-w-4xl mt-2 sm:mt-10',
  ...
  loading: {
    spinner: 'border-2 border-white/30 border-t-white rounded-full animate-spin',
